...
# Store conversation history - a dictionary mapping channel IDs to lists of message pairs
# Using defaultdict to automatically create a new list for new channels
conversation_history = defaultdict(list)

# Maximum history items to keep per channel to avoid token limits
    ...
    if message.channel.id in active_channels and not message.content.startswith(bot.command_prefix):
        ...
        async with ctx.typing():
            try:
                ...
                response = await generate_content_with_history(message.channel.id, prompt, image_url)
...
async def generate_content_with_history(channel_id, prompt, image_url=None):
    """Generate content using Gemini model with conversation history."""
    logger.info(f"Generating content with history for channel {channel_id}")
    
    try:
        # Create a chat session with history support
        chat = genai_client.start_chat(
            model="gemini-2.0-flash-exp",
            history=[],  # Start with empty history that we'll populate
            config=types.GenerateContentConfig(
                response_modalities=["Text", "Image"]
            ),
        )
        
        # Prepare history in Gemini API format
        history_messages = []
        for history_item in conversation_history[channel_id]:
            # Add user message to history
            history_messages.append({
                "role": "user", 
                "parts": [{"text": history_item["user"]}]
            })
            
            # Add bot response to history
            history_messages.append({
                "role": "model", 
                "parts": [{"text": history_item["bot"]}]
            })
        
        # Set the chat history directly if there are messages
        if history_messages:
            logger.info(f"Adding {len(history_messages)} history messages to chat")
            chat.history = history_messages
        
        # Process current message with optional image
        if image_url:
            logger.info("Image URL provided for history-based generation")
            image_data = await download_image(image_url)
            if image_data:
        ...
        # Extract text response for history tracking
        text_response = ""
        ...
        if len(conversation_history[channel_id]) > MAX_HISTORY_ITEMS:
            conversation_history[channel_id] = conversation_history[channel_id][-MAX_HISTORY_ITEMS:]
        
        logger.info("Content generation with history successful")
        return response
    except Exception as e:
        logger.error(f"Error generating content with history: {e}")
        # Fallback to regular content generation without history
        logger.info("Falling back to history-less content generation")
        return await generate_content(prompt, image_url)
...
async def ask(ctx, *, prompt: str = None):
    ...
    async with ctx.typing():
        try:
            # Use the history-aware content generation
            response = await generate_content_with_history(ctx.channel.id, prompt, image_url)
...
@bot.command(name="clear_history", help="Clear the conversation history for the current channel")
async def clear_history(ctx):
    ...
    if channel_id in conversation_history:
        ...
        await ctx.send("Conversation history has been cleared for this channel.")
    else:
        await ctx.send("There was no conversation history to clear in this channel.")

@bot.command(name="history", help="Show the current conversation history")
async def history(ctx):
    """Show the current conversation history for the channel."""
    channel_id = ctx.channel.id
    
    if channel_id not in conversation_history or not conversation_history[channel_id]:
        await ctx.send("There's no conversation history in this channel yet.")
        return
    
    history_items = conversation_history[channel_id]
    history_text = "**Conversation History:**\n\n"
    
    for i, item in enumerate(history_items, 1):
        # Truncate messages if they're too long for display
        user_msg = item["user"]
        bot_msg = item["bot"]
...
`.clear_history` - Clear the conversation history for this channel
...
**Conversation Memory:**
- The bot remembers up to 10 previous exchanges in each channel
- This allows for more natural, context-aware conversations
- Use `.clear_history` if you want to start a fresh conversation
